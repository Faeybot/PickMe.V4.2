...
async def start_feed(callback: types.CallbackQuery, db: DatabaseService, state: FSMContext):
    ...
    text = (
        "📱 **UPDATE FEED CHANNEL**\n\n"
        "Bagikan aktivitas atau sapaanmu ke komunitas!\n"
        f"📝 Sisa kuota teks: **{max(0, sisa_teks)}/3**\n"
        f"📸 Sisa kuota foto: **{max(0, sisa_foto)}/1**\n\n"
        "Kirim **Teks** saja atau **Foto + Caption** sekarang:"
    )
...
async def process_feed_input(message: types.Message, state: FSMContext, db: DatabaseService):
    ...
    await message.answer("Bagaimana profilmu ingin ditampilkan?", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def finalize_feed_post(callback: types.CallbackQuery, state: FSMContext, db: DatabaseService):
    ...
    try:
        if data['f_type'] == "photo":
            await callback.bot.send_photo(os.getenv("FEED_CHANNEL_ID"), photo=data['f_file_id'], caption=full_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
            await db.increment_quota(user.id, "photo_post")
        else:
            await callback.bot.send_message(os.getenv("FEED_CHANNEL_ID"), full_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
            await db.increment_quota(user.id, "text_post")
        await callback.message.edit_text("✅ Berhasil diposting!")
